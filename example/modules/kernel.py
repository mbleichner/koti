...
def kernel(root_uuid: str, cachyos: bool) -> ConfigGroups: return [
  ConfigGroup(
  ...
  ConfigGroup(
    ...
    Package("efibootmgr"),
    Package("linux-cachyos") if cachyos else None,
    Package("linux-cachyos-headers") if cachyos else None,
    ...
    File("/boot/loader/loader.conf", permissions = 0o555, content = cleandoc(f'''
      # managed by koti
      default {"arch-cachyos.conf" if cachyos else "arch-stock.conf"}
      timeout 3
      console-mode 2
    ''')),
