...
class PreHookManager(ConfigManager[PreHook]):
  ...
  def execute_phase(self, items: list[PreHook], core: Core, state: ExecutionState):
    for hook in items:
      confirm(
...
class PostHookManager(ConfigManager[PostHook]):
  ...
  def execute_phase(self, items: list[PostHook], core: Core, state: ExecutionState):
    for hook in items:
  ...
  def has_triggered(self, hook: PostHook, core: Core, state: ExecutionState) -> bool:
