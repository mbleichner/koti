...
class GroupManager(ConfigManager[GroupAssignment, GroupAssignmentState]):
  ...
  def get_current_assignments(self, model: ConfigModel) -> list[GroupAssignment]:
    ...
    currently_managed_users = set([item.username for group in model.configs for item in group.provides if isinstance(item, GroupAssignment)])
  ...
  def finalize(self, model: ConfigModel, dryrun: bool):
    if not dryrun:
      usernames = set([item.username for group in model.configs for item in group.provides if isinstance(item, GroupAssignment)])
